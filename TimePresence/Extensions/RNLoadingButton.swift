...
open class RNLoadingButton: UIButton {
    ...
    /// set highlighted border color from interface builder
    @IBInspectable open var normalBorderColor: UIColor = UIColor.clear
    @IBInspectable open var highlightedBorderColor: UIColor = UIColor.clear
    ...
    fileprivate func configureControl(for state: UIControlState) {
        // change border color when highlited
    ...
    fileprivate func frameForActivityIndicator() -> CGRect {
        ...
        case .symantic:
            // adjust activity indicator depending on app lang
            frame.origin.y += self.activityIndicatorEdgeInsets.top
            if (AppConfig.currentLanguage == .arabic) {
                frame.origin.x += self.frame.width - self.activityIndicatorView.frame.width - self.activityIndicatorEdgeInsets.right
            } else {
